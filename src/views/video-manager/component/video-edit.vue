          ...
          <el-row>
            ...
            <el-col :span="16" :xl="18" :lg="14" :md="24" :sm="24" :xs="24">
              <div class="dataRight">
                ...
                <div class="author">
                  <span>主演：{{ state.editForm.actor }}</span>
                </div>
                <div class="author">
                  <span>导演：{{ state.editForm.director }}</span>
                </div>
                <div class="author">
                  <span>分类：</span>
...
import { getDetail, getChapterList, updateList, addChapter } from '@/api/video'
...
const chapterRules = reactive<FormRules>({
  title: [{ required: true, message: '剧集名称不能为空', trigger: 'blur' }],
  title_alias: [
    { required: true, message: '剧集别名不能为空', trigger: 'blur' },
  ],
})
onMounted(() => {
  reqDetail()
...
const getMoreNewChapter = async () => {
  ...
  if (res.code == 200) {
    ...
    if (!chapterLength) {
      ...
      return ElMessage.warning('数据为空，无最新剧集')
    }
    state.logData = `开始上传剧集...`
