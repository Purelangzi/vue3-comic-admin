<template>
  <my-table
    ...
    @cancel-dialog="cancelDialog(dialogFormRef)"
...
const getData = async () => {
  ...
  try {
    ...
    if (data.data.length === 1 && state.searchParams.name) {
      tableConfig!.total = 1
    } else {
...
const handleDeleteCategory = async (row: any) => {
  const { name, id } = row
  await dropCategory(id, name, myTableRef)
}
// 批量删除分类
...
const dropCategory = async (
  ...
  await ElMessageBox.confirm(
    ...
    .then(async () => {
      ...
      // 删除后的总条数
      const totalNum =
        (tableConfig.total as number) -
        (state.selectList.length ? state.selectList.length : 1)
      // 删除后的总页数
      ...
      state.searchParams.page =
        state.searchParams.page > totalPage
          ? totalPage
          : state.searchParams.page
      // 当前页数小于0时为1，也就是删除了列表的最后一条数据
      state.searchParams.page =
        state.searchParams.page < 1 ? 1 : state.searchParams.page
      getData()
    })
    .catch(() => {
    ...
    .finally(() => {
      ...
      state.selectList.length ??
        (tableRef!.value.tableRef as TableInstance).clearSelection()
    })
}
...
./data/novel-category./data/category./data/novel-category./data/category-data
